...
def _load_valid_set(path: str) -> set[int]:
    with open(path) as f:
...
def build_tensor(
    ...
    return x, agent_mask, is_conflict, target_idx


def process_split(parquet_path: str, labels_path: str, valid_path: str,
                  out_prefix: str, name: str) -> None:
    """Iterate all valid events, build tensors, write memmap arrays."""
    valid = _load_valid_set(valid_path)
    labels_df = pd.read_csv(labels_path)
    labels_df = labels_df[labels_df["Event_id"].isin(valid)]
    ...
    for rg_idx in range(num_rg):
        ...
        for eid, evt in rg_df.groupby("Event_id"):
            ...
            try:
                x_arr, mask_arr, is_conflict, target_idx = build_tensor(evt, label)
            except ValueError:
    ...
    size_mb = sum(Path(p).stat().st_size for p in [x_path, mask_path, y_path, target_path]) / (1024**2)
